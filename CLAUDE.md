...
- `rick/photos/index.html` gallery, reads `PHOTOS` from `../data.js`. Photo files live in `rick/photos/`. Segments are collapsible `<details>` strips in route order (segment 1 on top), all collapsed by default; photos inside a segment are chronological, oldest first (Shawn's call, Sept 14, 2026). A photo's segment comes from the daily log (which segment he was riding that date), not the plan's dates, and each strip's date line shows the dates he actually rode it (Sept 20, 2026). Each strip shows a cover photo chosen in `PICKS` (segment number to file name) near the top of the script; add a pick when a new segment gets photos, otherwise it falls back to the segment's newest photo.
...
### Photos
- Real photos live in `danceflowers/photos/` (Sept 22, 2026). Leah's originals go in `danceflowers/photos/originals/`, which is gitignored; the web copies are JPEGs made with PIL (quality 85, progressive). Current files: `hero-white-roses.jpg` (desktop hero, 1006x1055, the white rose corsage and boutonnière close-up), `hero-white-roses-m.jpg` (phone hero via `<picture>` under 720px, a tighter crop with the corsage at the top), `card-corsage.jpg` and `card-boutonniere.jpg` (600x450 and 606x454 crops of the same photo), `og-white-roses.jpg` (share image). The bouquet card is still a stock placeholder with the "Placeholder photo" tag; the footer note says so.
- Hero layout: the corsage is in the photo's lower left, so on desktop the eyebrow, headline, lead, and buttons sit in a 600px column on the RIGHT (margin-left auto) with the headline at 54px on one line; under 720px the text is full width and the phone crop puts the corsage above it. If the hero photo changes, revisit `object-position` (desktop 50% 85%, phone 35% 0) and that right column.
- Shawn also dropped a coral corsage photo (IMG_0733, 735px, soft) and a gold-dress corsage (IMG_0729, 239px, too small) in originals; not used yet. Ask for full-resolution exports from Photos rather than screenshots. The full couple photo (IMG_0728(1)) has two faces; do not publish it without Shawn confirming permission.
