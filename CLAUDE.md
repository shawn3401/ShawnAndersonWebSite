...
1. Shawn sends: onX ride card (distance, gain, loss, high/low point; climbing is replaced by Rick's bike computer number whenever he reports one, see below), the wide onX map screenshot (green = ridden, purple = plan), his coordinates (DMS like 47°50'48.5"N 115°53'23.4"W; convert to decimal), optional photos (ASK which day they are from; batches often arrive a day late), and whether he rerouted (`course`).
   - Climbing source (Shawn, Sept 23, 2026): the site's daily `gain` matches Rick's bike computer, even where it is lower than onX, so he never feels shorted or confused. Use onX until Rick reports, then swap the day's `gain` and the full-day figures in its notes and final check-in; leave mid-day check-ins alone. Distance stays onX (it matches). As of Sept 23, still waiting on Rick for Sept 6, 8, 9, 10, 11 (his 2,589 for 9/11 was off; ignore it), 13 and 17.
...
- Footer with a link home and a small "Leah's orders" link to the admin sign-in.
...
### Photos
- Real photos live in `danceflowers/photos/` (Sept 22, 2026), all cut from Leah's photographer's full-resolution camera files (Nikon, `Z72_####.jpg`, 15 to 30 MB each). The camera files stay in `danceflowers/photos/originals/`, which is gitignored; NEVER commit them. Web copies are JPEGs made with PIL (quality 84, progressive). Shawn's rule: NO FACES on the public page. Every crop below is face-free; check any new crop for a face before publishing.
